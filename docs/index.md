...
for the clustering. The
[`stats::hclust()`](https://rdrr.io/r/stats/hclust.html) function expect
a dissimilarity structure as it’s first parameter (the result of
[`stats::dist()`](https://rdrr.io/r/stats/dist.html)). Since `hclust`
determines cluster membership for all *n* - 1 possibilities, we use the
[`stats::cutree()`](https://rdrr.io/r/stats/cutree.html) function for
